...
export type PhotoUrlFactory = (managedFilename: string | null) => string | null;

const CATALOG_PAGE_SIZE = 25;

export async function listCatalogPage(
  catalogRepository: PlantCatalogRepository,
  createPhotoUrl: PhotoUrlFactory,
  ...
  return {
    items: result.items.map((plant) => toCatalogPlant(plant, createPhotoUrl)),
    page,
...
export async function listSelectionSummaries(
  ...
  return (await selectionRepository.listSummaries()).map((selection) =>
    toSelectionSummary(selection, createPhotoUrl),
  );
}
...
export async function getSelectionDetails(
  ...
  return toSelectionDetails(selection, createPhotoUrl);
}
...
export async function acknowledgeDeletedSelectionPlants(
  ...
  return selection ? toSelectionDetails(selection, createPhotoUrl) : null;
}
...
function toSelectionDetails(
  ...
  return {
    ...
    plants: selection.plants.map((plant) =>
      toCatalogPlant(plant, createPhotoUrl),
    ),
  };
}
