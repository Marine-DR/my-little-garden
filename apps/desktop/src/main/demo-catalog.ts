...
function integer(value: string | undefined): number | null {
  ...
  if (!Number.isInteger(parsed)) throw new Error(`Invalid integer value: ${text}`);
  return parsed;
}
...
export function seedDemoCatalog(database: DatabaseSync, csv: string): number {
  ...
  try {
    for (const row of rows) {
      ...
      const kind: PlantKind | null = kindKey === 'fleur' ? 'flower' : kindKey === 'feuillage' ? 'foliage'
      ...
      if (heightMin === null && heightMax !== null) {
        throw new Error(`Plant ${name} has a maximum height without a minimum height.`);
      }
      ...
      const input: PlantWriteInput = {
        ...
        bloom: bloomStart !== null && bloomEnd !== null
      ...
      database.prepare(`INSERT INTO plants (
        ...
        .run(id, name, normalizeDatabaseKey(name), heightMin, heightMax,
          type ? vocabularyId(database, 'plant_types', type) : null, kind,
          bloomStartValue, bloomEndValue, input.minimumTemperatureCelsius,
          persistence, input.spacingCm, now, now);
